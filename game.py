...
class Game:
    ...
    def get_stat(self):
        ...
        if self.game_winner is True:
            stat += "White player won\n"
        elif self.game_winner is False:
            stat += "Black player won\n"
        elif self.draw:
            stat += "A draw\n"
        stat += ("End score: " + str(self.end_score[0]) + ":" + str(self.end_score[1]) + "White:Black\n")
        stat += ("Game time: " + str(self.game_time) + "ms\nTurn count: " + str(self.turn_count) + "\n")
        stat += ("White player average move time: "
              + str(self.average_white_move_time) + "ms\n")
        stat += ("Longest white player move time: " + str(self.max_white_move_time) + "ms\n")
        stat += ("Black player average move time: "
              + str(self.average_black_move_time) + "ms\n")
        stat += ("Longest black player move time: " + str(self.max_black_move_time) + "ms\n")
        return stat
